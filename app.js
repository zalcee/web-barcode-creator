        ...
        <head>
            ...
            <style>
                ...
                .container {
                    ...
                    text-align: center;
                    max-width: 90%;
                    width: 450px; /* Slightly wider container for upload area */
                    margin-top: 2rem;
                }
...
app.post('/upload', upload.single('file'), async (req, res) => {
    ...
    fs.readdir(barcodesDir, (err, files) => {
        if (err) {
            ...
            // Even if there's an error reading, we might want to proceed to try and create new barcodes
            // Or decide to return an error depending on how critical it is to clear
        } else {
    ...
    const labelsPerRow = 5; // Number of labels horizontally
    const labelWidth = 100; // Width of each label area
    const labelHeight = 100; // Height of each label area (adjust if content overflows)
    const paddingX = 0; // Horizontal padding between labels
    const paddingY = 0; // Vertical padding between labels
    ...
    labels.forEach((item, index) => {
        ...
        // Set line width for the border to make it thinner
        doc.lineWidth(0.5); // Set line width to 0.5 (default is 1)
        // Draw a rectangle as a bounding box for the label (useful for debugging layout)
        doc.rect(x, y, labelWidth, labelHeight).stroke();
        doc.lineWidth(1); // Reset line width to default for other potential drawing operations

        // Draw KCC logo or text
        // if (fs.existsSync('KCC_Mall_Logo_bw.PNG')) {
        //     // If 'logo.png' exists, embed it
        //     doc.image('KCC_Mall_Logo_bw.PNG', x + 18, y + 5, { width: 32, height: 18 });
        // } else {
        //     // Otherwise, draw 'KCC' text
        //     doc.fontSize(6).text('KCC', x + 5, y + 5);
        //     // doc.image('KCC_Mall_Logo.png', x + 18, y + 5, { width: 32, height: 18 });
        // }




        const KCC_BW_Logo_Path = 'KCC_Mall_logo_bw.PNG'; // Corrected to match found filename
        const KCC_Logo_Path = 'KCC_Mall_logo.png';       // Assuming this is still the correct case for the other logo

        console.log(`Checking for ${KCC_BW_Logo_Path}: ${fs.existsSync(KCC_BW_Logo_Path)}`);
        console.log(`Checking for ${KCC_Logo_Path}: ${fs.existsSync(KCC_Logo_Path)}`);


        if (fs.existsSync(KCC_BW_Logo_Path)) {
            console.log(`Embedding ${KCC_BW_Logo_Path}`);
            try {
                doc.image(KCC_BW_Logo_Path, x + 18, y + 5, { width: 32, height: 18 });
            } catch (imageError) {
                console.error(`Error embedding ${KCC_BW_Logo_Path}:`, imageError);
            }
        } else {
            console.log(`Attempting to embed KCC text because ${KCC_BW_Logo_Path} was not found.`);
            doc.fontSize(6).text('KCC', x + 5, y + 5);
            // If you intend to use KCC_Mall_logo.png here, uncomment and ensure path is correct:
            // console.log(`Attempting to embed ${KCC_Logo_Path}`);
            // try {
            //     doc.image(KCC_Logo_Path, x + 18, y + 5, { width: 32, height: 18 });
            // } catch (imageError) {
            //     console.error(`Error embedding ${KCC_Logo_Path}:`, imageError);
            // }
        }
        ...
        // Draw SUPP and STOCK information
        // Use lineGap: 0 to ensure lines are close together for small font size
        doc.font('Helvetica').fontSize(7).text(`${item.supp}\n${item.sku}`, x + 30, y + 5, { width: labelWidth - 35, lineGap: 0, align: 'right' });
        ...
        // Draw barcode number in bold
        doc.font('Helvetica') // Set font to bold before drawing barcode text
           .fontSize(7)
           .text(item.barcode, x + 2, y + 60, { width: labelWidth - 4, align: 'center' });
        doc.font('Helvetica'); // Reset font to regular Helvetica for subsequent text

        // Draw description
        doc.font('Helvetica-Bold').fontSize(5.33).text(item.description, x + 2, y + 69, { width: labelWidth - 4, align: 'center' });
